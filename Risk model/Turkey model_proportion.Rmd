...
get_table_values <- function(full_results_table) {
  specific_table <- data.frame(
    scenario = c(rep(c("Over the level threshold", "Under the level threshold", "Inclusion list&over the level threshold", "Inclusion list|over the level threshold", "Typhimurim-Vaccine&over the level threshold", "Not-Typhimurium-Vaccine&over the level threshold"), each=3)),
    threshold = rep(
      c(
...
#microbial_threshold <- list(T1=1/(30*(1814/400)), T2=1, T3=10)
  ...
  if (model_mode==1) {
    ...
    anatum_alpha <- 0.318
    anatum_beta <- 4729.90
    if (stringr::str_sub(serotype_in_serving, 1, -2)=="Vaccined") {
...
m3_variable_attenuation_scaled <- poultry_model_clustered(
  ...
  use_attenuation = TRUE, # Use attenuation?
  use_scaling_factor = TRUE, # Use scaling factor?; DO NOT CHANGE
  comment = "m3_variable_prop_2024_list1_updatedsero" # User/Scenario comment; HIGHLY RECOMMENDED TO EDIT AS APPROPRIATE
)
m3_variable_attenuation_scaled_specifics <- get_table_values(m3_variable_attenuation_scaled) # Extracts specific data needed for Kim et al. 2024 paper
mean(m3_variable_attenuation_scaled$tdr_mean_pr_ill)
```
