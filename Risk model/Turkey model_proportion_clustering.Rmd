---
title: "Poultry Model for Turkey proportion DR"
author: "Cecil Barnett-Neefs & Yiyi Li"
date: "2024-09-10"
...
serotype_probabilities_2021 <- readxl::read_xlsx("Serotype_Probabilities.xlsx", sheet = "HACCP 2016-2021 (prop)") # HACCP baseline 2016-2021 "Sheet1" or new 2023-2024 "Sheet2" 
serotype_probabilities_2024 <- readxl::read_xlsx("Serotype_Probabilities.xlsx", sheet = "HACCP 2023-2024 (prop)") # HACCP baseline 2016-2021 "Sheet1" or new 2023-2024 "Sheet2" 
serotype_probabilities_2021 <- serotype_probabilities_2021 %>% 
    dplyr::select(Serotype, `Cluster1`, `Cluster2`)%>%
  pivot_longer(cols = c(Cluster1, Cluster2), 
               names_to = "Cluster", 
               values_to = "Probability") %>% 
  mutate(Serotype_Cluster = paste0(Serotype, ifelse(Cluster == "Cluster1","1","2")),
         Probability = ifelse(Probability != 0, Probability, 0)) %>% 
  dplyr::select(Serotype_Cluster, Probability)  
serotype_probabilities_2024 <- serotype_probabilities_2024 %>% 
    dplyr::select(Serotype, `Cluster1`, `Cluster2`)%>%
  pivot_longer(cols = c(Cluster1, Cluster2), 
               names_to = "Cluster", 
               values_to = "Probability") %>% 
...
tuenis_submodel_parameters <- readxl::read_xlsx("Tuenis_Submodel_Parameters_updated.xlsx", sheet = "Sheet1")
serotype_equivalents <- readxl::read_xlsx("Serotype_Equivalents_updated.xlsx", sheet = "Sheet1")
...
get_table_values <- function(full_results_table) {
  specific_table <- data.frame(
    scenario = c(rep(c("Over the level threshold", "Under the level threshold", "Inclusion list&over the level threshold", "Inclusion list|over the level threshold", "Exclusion&over the level threshold", "Not-Exclusion&over the level threshold"), each=3)),
    threshold = rep(
      c(
    ...
    proportion_meeting_product_characteristic = c(
      ...
      mean(full_results_table$proportion_noncompliant_with_mc_1_or_inclusion_list, na.rm = TRUE),
      mean(full_results_table$proportion_noncompliant_with_mc_2_or_inclusion_list, na.rm = TRUE),
      mean(full_results_table$proportion_noncompliant_with_mc_3_or_inclusion_list, na.rm = TRUE),
    ...
    number_of_servings_meeting_product_characteristic = c(
      ...
      mean(full_results_table$number_of_servings_mc_1_noncompliant_or_from_inclusion_list, na.rm = TRUE),
      mean(full_results_table$number_of_servings_mc_2_noncompliant_or_from_inclusion_list, na.rm = TRUE),
      mean(full_results_table$number_of_servings_mc_3_noncompliant_or_from_inclusion_list, na.rm = TRUE),
    ...
    average_p_ill = c(
      ...
      mean(full_results_table$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_mean, na.rm = TRUE),
      mean(full_results_table$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_mean, na.rm = TRUE),
      mean(full_results_table$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_mean, na.rm = TRUE),
    ...
    illness_proportion_from_characterized_product = c(
      ...
      mean(full_results_table$illness_proportion_from_inclusion_list_or_level_based_mc_1_noncompliant, na.rm = TRUE),
      mean(full_results_table$illness_proportion_from_inclusion_list_or_level_based_mc_2_noncompliant, na.rm = TRUE),
      mean(full_results_table$illness_proportion_from_inclusion_list_or_level_based_mc_3_noncompliant, na.rm = TRUE),
...
teunis_dose_response <- function(sample_size, sample_serotype, tuenis_submodel_parameters, serotype_equivalents, dose=1, debug=FALSE) {
  equivalent_serotype_row <- which(serotype_equivalents$selected_serotype==ifelse(grepl("1$", sample_serotype),stringr::str_sub(sample_serotype, 1, -2), sample_serotype))
  if (length(equivalent_serotype_row) > 0) {
    if (debug==TRUE) {
      print(paste0("Matched Serotype: ", stringr::str_sub(sample_serotype, 1, -2))) 
    }
    ...
    if (grepl("1$", sample_serotype)){
        equivalent_serotype <- c("Enteritidis", "Typhimurium")[sample(2,1)]
      }else if (grepl("2$", sample_serotype)){
        equivalent_serotype <- c("Anatum")
          }
  ...
  # If dose is too small use natural log to return a non-zero value which can be forced to higher precision
  if (infection_vector == 0) {
    infection_vector <- as.numeric(1-exp(mpfr(BAS::hypergeometric1F1(a, a+b, -dose, log=TRUE), 100)))
  }
  
  if (infection_vector <= 0) {
    infection_vector <- 1E-20
    p_fix <- TRUE
  }
  # Create bivariate normal distribution
...
poultry_model_clustered <- function(total_iterations, total_simulations, cores=detectCores(), model_mode, serotype_probabilities, serotype_equivalents, salmonella_mean, salmonella_st_dev, attenuation_input, inclusion_list_input, exclusion_list_input, scaling_factor, product_consumed_per_individual_per_day_mean, product_consumed_per_individual_per_day_st_dev, annual_servings, teunis_dose_response, tuenis_submodel_parameters, pr_ill_from_dose, poultry_model, save_runs=TRUE, use_attenuation=TRUE, use_scaling_factor=TRUE, comment="") {
  ...
  if (input_check==TRUE) {
    ...
    while(i <= total_simulations) {
      ...
      #add seed "i" in model_runs
      model_runs <- parLapply(cl, 1:total_iterations, poultry_model, total_iterations,i ,model_mode, serotype_probabilities, serotype_equivalents, salmonella_mean, salmonella_st_dev, attenuation_input, inclusion_list_input, exclusion_list_input, scaling_factor,  product_consumed_per_individual_per_day_mean, product_consumed_per_individual_per_day_st_dev, teunis_dose_response, tuenis_submodel_parameters, pr_ill_from_dose, use_attenuation, use_scaling_factor)
      stopCluster(cl)
      ...
      simulation_run <- data.frame(
        ...
        attenuation_value = mean(model_runs_tidy$attenuation_value),
        ...
        proportion_noncompliant_with_mc_1_or_inclusion_list = mean(model_runs_tidy$is_level_above_threshold_1_or_from_inclusion_list, na.rm = TRUE), # Proportion above threshold level, or serotype on inclusion list
        proportion_noncompliant_with_mc_1_exclusion_list = mean(model_runs_tidy$is_level_above_threshold_1_and_from_exclusion_list, na.rm = TRUE), # Proportion Typhimurium - Possible Vaccine Strain&over level threshold
        proportion_noncompliant_with_mc_1_not_exclusion_list = mean(model_runs_tidy$is_serotype_above_threshold_1_and_from_not_exclusion_list, na.rm = TRUE), # Proportion not Typhimurim-Vaccine&over the level threshold
        ...
        # T1|Inclusion List+Compliant
        pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product, probs = 0.95, na.rm = TRUE),
        # T1|Inclusion List+Compliant Log10
        pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_log10_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_log10, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_log10_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_log10, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_log10_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_compliant_product_log10, probs = 0.95, na.rm = TRUE),
        # T1|Inclusion List+Noncompliant/P(ill) from Inclusion list & level over the threshold
        pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product, probs = 0.95, na.rm = TRUE),
        # T1|Inclusion List+Noncompliant Log10/P(ill) from Inclusion list & level over the threshold
        pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_log10_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_log10, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_log10_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_log10, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_log10_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_log10, probs = 0.95, na.rm = TRUE),
        ...
        # T1+Not Exclusion List+Noncompliant/P(ill) from Serotype not Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        # T1+Not Exclusion List+Noncompliant Log10/P(ill) from Serotype not Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        proportion_noncompliant_with_mc_2_or_inclusion_list = mean(model_runs_tidy$is_level_above_threshold_2_or_from_inclusion_list, na.rm = TRUE), # Proportion above threshold level, or serotype on inclusion list
        proportion_noncompliant_with_mc_2_exclusion_list = mean(model_runs_tidy$is_level_above_threshold_2_and_from_exclusion_list, na.rm = TRUE), # Proportion Typhimurium - Possible Vaccine Strain&over level threshold
        proportion_noncompliant_with_mc_2_not_exclusion_list = mean(model_runs_tidy$is_serotype_above_threshold_2_and_from_not_exclusion_list, na.rm = TRUE), # Proportion not Typhimurim-Vaccine&over the level threshold
        ...
        # T2|Inclusion List+Compliant
        pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product, probs = 0.95, na.rm = TRUE),
        # T2|Inclusion List+Compliant Log10
        pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_log10_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_log10, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_log10_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_log10, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_log10_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_compliant_product_log10, probs = 0.95, na.rm = TRUE),
        # T2|Inclusion List+Noncompliant/P(ill) from Inclusion list & level over the threshold
        pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product, probs = 0.95, na.rm = TRUE),
        # T2|Inclusion List+Noncompliant Log10/P(ill) from Inclusion list & level over the threshold
        pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_log10_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_log10, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_log10_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_log10, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_log10_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_log10, probs = 0.95, na.rm = TRUE),
        ...
        # T2+Exclusion List+Noncompliant/P(ill) from Serotype Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        # T2+Exclusion List+Noncompliant Log10/P(ill) from Serotype Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        # T2+Not Exclusion List+Noncompliant/P(ill) from Serotype not Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        # T2+Not Exclusion List+Noncompliant Log10/P(ill) from Serotype not Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        proportion_noncompliant_with_mc_3_or_inclusion_list = mean(model_runs_tidy$is_level_above_threshold_3_or_from_inclusion_list, na.rm = TRUE), # Proportion above threshold level, or serotype on inclusion list
        proportion_noncompliant_with_mc_3_exclusion_list = mean(model_runs_tidy$is_level_above_threshold_3_and_from_exclusion_list, na.rm = TRUE), # Proportion Typhimurium - Possible Vaccine Strain&over level threshold
        proportion_noncompliant_with_mc_3_not_exclusion_list = mean(model_runs_tidy$is_serotype_above_threshold_3_and_from_not_exclusion_list, na.rm = TRUE), # Proportion not Typhimurim-Vaccine&over the level threshold
        ...
        # T3|Inclusion List+Compliant
        pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product, probs = 0.95, na.rm = TRUE),
        # T3|Inclusion List+Compliant Log10
        pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_log10_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_log10, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_log10_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_log10, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_log10_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_compliant_product_log10, probs = 0.95, na.rm = TRUE),
        # T3|Inclusion List+Noncompliant/P(ill) from Inclusion list & level over the threshold
        pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product, probs = 0.95, na.rm = TRUE),
        # T3|Inclusion List+Noncompliant Log10/P(ill) from Inclusion list & level over the threshold
        pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_log10_mean = mean(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_log10, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_log10_5th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_log10, probs = 0.05, na.rm = TRUE),
        pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_log10_95th = quantile(model_runs_tidy$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_log10, probs = 0.95, na.rm = TRUE),
        ...
        # T3+Exclusion List+Noncompliant/P(ill) from Serotype Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        # T3+Exclusion List+Noncompliant Log10/P(ill) from Serotype Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        # T3+Not Exclusion List+Noncompliant/P(ill) from Serotype not Typhimurium - Possible Vaccine Strain & level over the threshold
        ...
        # T3+Not Exclusion List+Noncompliant Log10/P(ill) from Serotype not Typhimurium - Possible Vaccine Strain & level over the threshold
      ...
      # P(ill) from Inclusion list | level over the threshold-Relative Risk
      simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_relative_risk <- simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_mean/baseline
      simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_relative_risk <- simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_mean/baseline
      simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_relative_risk <- simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_mean/baseline
      ...
      # Inclusion list|over the level threshold
      simulation_run$number_of_servings_mc_1_noncompliant_or_from_inclusion_list <- annual_servings*simulation_run$proportion_noncompliant_with_mc_1_or_inclusion_list
      simulation_run$number_of_servings_mc_2_noncompliant_or_from_inclusion_list <- annual_servings*simulation_run$proportion_noncompliant_with_mc_2_or_inclusion_list
      simulation_run$number_of_servings_mc_3_noncompliant_or_from_inclusion_list <- annual_servings*simulation_run$proportion_noncompliant_with_mc_3_or_inclusion_list
      # Typhimurim-Vaccine&over the level threshold
      ...
      # Not-Typhimurium-Vaccine&over the level threshold
      ...
      # Inclusion list|over the level threshold
      simulation_run$illness_proportion_from_inclusion_list_or_level_based_mc_1_noncompliant <- simulation_run$number_of_servings_mc_1_noncompliant_or_from_inclusion_list*simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_1_noncompliant_product_mean/simulation_run$annual_predicted_illnesses
      simulation_run$illness_proportion_from_inclusion_list_or_level_based_mc_2_noncompliant <- simulation_run$number_of_servings_mc_2_noncompliant_or_from_inclusion_list*simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_2_noncompliant_product_mean/simulation_run$annual_predicted_illnesses
      simulation_run$illness_proportion_from_inclusion_list_or_level_based_mc_3_noncompliant <- simulation_run$number_of_servings_mc_3_noncompliant_or_from_inclusion_list*simulation_run$pr_ill_from_inclusion_list_or_level_based_mc_3_noncompliant_product_mean/simulation_run$annual_predicted_illnesses
      # Typhimurim-Vaccine&over the level threshold
...
poultry_model <- function(i, total_iterations,simulation_batch, model_mode, serotype_probabilities, serotype_equivalents, salmonella_mean, salmonella_st_dev, attenuation_input,inclusion_list_input, exclusion_list_input, scaling_factor, product_consumed_per_individual_per_day_mean, product_consumed_per_individual_per_day_st_dev, teunis_dose_response, tuenis_submodel_parameters, pr_ill_from_dose, use_attenuation=TRUE, use_scaling_factor=TRUE) {
   set.seed(i+(total_iterations*(simulation_batch-1)), "Mersenne-Twister")
#microbial_threshold <- list(T1=1/(30*(1814/400)), T2=1, T3=10)
  microbial_threshold <- list(T1=1/325, T2=1, T3=10) #change for Communited turkey LOD is 1/325 
  serotype_inclusion_list <- inclusion_list_input  
  serotype_exclusion_list <- exclusion_list_input 
  ...
  # Inclusion List
  # Is serotype on Inclusion List?
  is_serotype_on_inclusion_list <- any(grepl(stringr::str_sub(serotype_in_serving, 1, -2), serotype_inclusion_list)) #Inclusion list 1 and 2 
  #is_serotype_on_inclusion_list <- any(grepl(serotype_in_serving, serotype_inclusion_list)) #Inclusion list 3
  ...
  # OR Inclusion List 
  is_level_above_threshold_1_or_from_inclusion_list <- as.logical(is_level_above_threshold_1|is_level_above_threshold_1_and_from_inclusion_list) # Is level above threshold OR from inclusion list subtype(>LOD)?
  is_level_above_threshold_2_or_from_inclusion_list <- as.logical(is_level_above_threshold_2|is_level_above_threshold_1_and_from_inclusion_list) # Is level above threshold OR from inclusion list subtype(>LOD)?
  is_level_above_threshold_3_or_from_inclusion_list <- as.logical(is_level_above_threshold_3|is_level_above_threshold_1_and_from_inclusion_list) # Is level above threshold OR from inclusion list subtype(>LOD)?

  # Exclusion List
  # Is serotype on Exclusion List(=Typhimurium - Possible Vaccine Strain)?
  is_serotype_on_exclusion_list <- any(grepl(stringr::str_sub(serotype_in_serving, 1, -2), serotype_exclusion_list)) #Inclusion list 1 and 2 
  #is_serotype_on_exclusion_list <- any(grepl(serotype_in_serving, serotype_exclusion_list)) #Inclusion list 3
  
  is_level_above_threshold_1_and_from_exclusion_list <- as.logical(is_level_above_threshold_1*is_serotype_on_exclusion_list) # Is level above threshold AND serotype Typhimurium - Possible Vaccine Strain?
  is_level_above_threshold_2_and_from_exclusion_list <- as.logical(is_level_above_threshold_2*is_serotype_on_exclusion_list) # Is level above threshold AND serotype Typhimurium - Possible Vaccine Strain?
  is_level_above_threshold_3_and_from_exclusion_list <- as.logical(is_level_above_threshold_3*is_serotype_on_exclusion_list) # Is level above threshold AND serotype Typhimurium - Possible Vaccine Strain?

  is_serotype_not_on_exclusion_list <- !is_serotype_on_exclusion_list # Is serotype on Not Exclusion List(=Typhimurium - Possible Vaccine Strain)?
  ...
  # T1|Inclusion List
  if (is_level_above_threshold_1_or_from_inclusion_list) {
    inclusion_list_or_level_based_mc_1_compliant_dose <- NA
    inclusion_list_or_level_based_mc_1_noncompliant_dose <- dose_in_one_raw_serving
  } else {
    inclusion_list_or_level_based_mc_1_compliant_dose <- dose_in_one_raw_serving
    inclusion_list_or_level_based_mc_1_noncompliant_dose <- NA
  }
  # T1+Exclusion List
  ...
  # T2|Inclusion List
  if (is_level_above_threshold_2_or_from_inclusion_list) {
    inclusion_list_or_level_based_mc_2_compliant_dose <- NA
    inclusion_list_or_level_based_mc_2_noncompliant_dose <- dose_in_one_raw_serving
  } else {
    inclusion_list_or_level_based_mc_2_compliant_dose <- dose_in_one_raw_serving
    inclusion_list_or_level_based_mc_2_noncompliant_dose <- NA
  }
  # T2+Exclusion List
  ...
  # T3|Inclusion List
  if (is_level_above_threshold_3_or_from_inclusion_list) {
    inclusion_list_or_level_based_mc_3_compliant_dose <- NA
    inclusion_list_or_level_based_mc_3_noncompliant_dose <- dose_in_one_raw_serving
  } else {
    inclusion_list_or_level_based_mc_3_compliant_dose <- dose_in_one_raw_serving
    inclusion_list_or_level_based_mc_3_noncompliant_dose <- NA
  }
  # T3+Exclusion List
  ...
  if (use_attenuation) {
    if (length(attenuation_input) == 2) {
      attenuation_value <- 10^rnorm(1, attenuation_input[1], attenuation_input[2])
    } else if (length(attenuation_input) == 1) {
      attenuation_value <- attenuation_input
    } else {
      attenuation_value <- 1
    }
  }
  if (use_scaling_factor) {
    ...
    scaling_factor_final <- 1
  }
  ## Dose with scaling factor ----
  dose_exposed_to_host_post_scaled <- scaling_factor_final*attenuation_value*dose_in_one_raw_serving
  ...
  if (model_mode==1) {
    ...
    anatum_alpha <- 0.318
    anatum_beta <- 4729.90
    if (stringr::str_sub(serotype_in_serving, 1, -2)=="Kentucky") {
  ...
  # T1+Compliant
  mc_threshold_1_compliant_dose_scaled <- scaling_factor_final*attenuation_value*mc_threshold_1_compliant_dose
  # T1+Noncompliant
  mc_threshold_1_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*mc_threshold_1_noncompliant_dose
  # T1+Inclusion List+Compliant
  inclusion_list_and_level_based_mc_1_compliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_and_level_based_mc_1_compliant_dose
  # T1+Inclusion List+Noncompliant
  inclusion_list_and_level_based_mc_1_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_and_level_based_mc_1_noncompliant_dose
  # T1|Inclusion List+Compliant
  inclusion_list_or_level_based_mc_1_compliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_or_level_based_mc_1_compliant_dose
  # T1|Inclusion List+Noncompliant
  inclusion_list_or_level_based_mc_1_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_or_level_based_mc_1_noncompliant_dose
  # T1+Exclusion List+Compliant
  exclusion_list_and_level_based_mc_1_compliant_dose_scaled <- scaling_factor_final*attenuation_value*exclusion_list_and_level_based_mc_1_compliant_dose
  # T1+Exclusion List+Noncompliant
  exclusion_list_and_level_based_mc_1_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*exclusion_list_and_level_based_mc_1_noncompliant_dose
  # T1+Not Exclusion List+Compliant
  not_exclusion_list_and_level_based_mc_1_compliant_dose_scaled <- scaling_factor_final*attenuation_value*not_exclusion_list_and_level_based_mc_1_compliant_dose
  # T1+Not Exclusion List+Noncompliant
  not_exclusion_list_and_level_based_mc_1_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*not_exclusion_list_and_level_based_mc_1_noncompliant_dose
  
  # T2+Compliant
  mc_threshold_2_compliant_dose_scaled <- scaling_factor_final*attenuation_value*mc_threshold_2_compliant_dose
  # T2+Noncompliant
  mc_threshold_2_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*mc_threshold_2_noncompliant_dose
  # T2+Inclusion List+Compliant
  inclusion_list_and_level_based_mc_2_compliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_and_level_based_mc_2_compliant_dose
  # T2+Inclusion List+Noncompliant
  inclusion_list_and_level_based_mc_2_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_and_level_based_mc_2_noncompliant_dose
  # T2|Inclusion List+Compliant
  inclusion_list_or_level_based_mc_2_compliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_or_level_based_mc_2_compliant_dose
  # T2|Inclusion List+Noncompliant
  inclusion_list_or_level_based_mc_2_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_or_level_based_mc_2_noncompliant_dose
  # T2+Exclusion List+Compliant
  exclusion_list_and_level_based_mc_2_compliant_dose_scaled <- scaling_factor_final*attenuation_value*exclusion_list_and_level_based_mc_2_compliant_dose
  # T2+Exclusion List+Noncompliant
  exclusion_list_and_level_based_mc_2_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*exclusion_list_and_level_based_mc_2_noncompliant_dose
  # T2+Not Exclusion List+Compliant
  not_exclusion_list_and_level_based_mc_2_compliant_dose_scaled <- scaling_factor_final*attenuation_value*not_exclusion_list_and_level_based_mc_2_compliant_dose
  # T2+Not Exclusion List+Noncompliant
  not_exclusion_list_and_level_based_mc_2_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*not_exclusion_list_and_level_based_mc_2_noncompliant_dose
  
  # T3+Compliant
  mc_threshold_3_compliant_dose_scaled <- scaling_factor_final*attenuation_value*mc_threshold_3_compliant_dose
  # T3+Noncompliant
  mc_threshold_3_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*mc_threshold_3_noncompliant_dose
  # T3+Inclusion List+Compliant
  inclusion_list_and_level_based_mc_3_compliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_and_level_based_mc_3_compliant_dose
  # T3+Inclusion List+Noncompliant
  inclusion_list_and_level_based_mc_3_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_and_level_based_mc_3_noncompliant_dose
  # T3|Inclusion List+Compliant
  inclusion_list_or_level_based_mc_3_compliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_or_level_based_mc_3_compliant_dose
  # T3|Inclusion List+Noncompliant
  inclusion_list_or_level_based_mc_3_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*inclusion_list_or_level_based_mc_3_noncompliant_dose
  # T3+Exclusion List+Compliant
  exclusion_list_and_level_based_mc_3_compliant_dose_scaled <- scaling_factor_final*attenuation_value*exclusion_list_and_level_based_mc_3_compliant_dose
  # T3+Exclusion List+Noncompliant
  exclusion_list_and_level_based_mc_3_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*exclusion_list_and_level_based_mc_3_noncompliant_dose
  # T3+Not Exclusion List+Compliant
  not_exclusion_list_and_level_based_mc_3_compliant_dose_scaled <- scaling_factor_final*attenuation_value*not_exclusion_list_and_level_based_mc_3_compliant_dose
  # T3+Not Exclusion List+Noncompliant
  not_exclusion_list_and_level_based_mc_3_noncompliant_dose_scaled <- scaling_factor_final*attenuation_value*not_exclusion_list_and_level_based_mc_3_noncompliant_dose
...
```{r Variable Attenuation & Scaled Run for comminuted turkey, HACCP baseline (2016-2021) list 1}
# Parameter setup ----
...
salmonella_st_dev <- 2.052 # Now is Bayesian fitting, for FSIS HACCP Data 2016-2021, use 2.333
...
m3_variable_attenuation_scaled <- poultry_model_clustered(
  ...
  attenuation_input = c(-5, 1.91), # FSIS Variable attenuation, Quantitative Risk Assessment for Salmonella in Raw Chicken and Raw Chicken Products, page 42
   inclusion_list_input=c("Typhimurium", "Hadar", "Muenchen"),# Change the inclusion list based on different scenarios; if change to list 3, remember to change the stringr::str_sub for the inclusion/exclusion list
  ...
  exclusion_list_input=c("Kentucky"), #c("Typhimurium - Possible Vaccine Strain", change to "Vaccined" in the proportion excel)
  ...
  annual_servings = 10983387032, #28482586137, # total turkey servings per year in the U.S; POPULATION*NUMBER OF SERVINGS PER PERSON, CURRENTLY USES 2023 DATA #10983387032 for ground turkey 
  ...
  comment = "m3_variable_prop_2021_list1_sf2_check" # User/Scenario comment; HIGHLY RECOMMENDED TO EDIT AS APPROPRIATE
)
...
```{r Variable Attenuation & Scaled Run for comminuted turkey, HACCP baseline (2016-2021) list 2}
# Parameter setup ----
# NOTE: You can put these number straight into poultry_model_clustered() instead of defining them here, but this method lets you cite data sources more cleanly
salmonella_mean <- -4.857 #FSIS HACCP Data 2016-2021
salmonella_st_dev <- 2.052 # Now is Bayesian fitting, for FSIS HACCP Data 2016-2021, use 2.333
...
m3_variable_attenuation_scaled <- poultry_model_clustered(
  ...
  salmonella_st_dev = salmonella_st_dev, # Log10 Salmonella dose standard deviation; CAN BE CHANGED WITH A DIFFERENT VALUE
  attenuation_input = c(-5, 1.91), # FSIS Variable attenuation, Quantitative Risk Assessment for Salmonella in Raw Chicken and Raw Chicken Products, page 42
   inclusion_list_input=c("Typhimurium", "Hadar", "Muenchen","I 4,[5],12:i:-","Berta","Newport", "Saintpaul", "Enteritidis", "Litchfield", "Manhattan"),# Change the inclusion list based on different scenarios; if change to list 3, remember to change the stringr::str_sub for the inclusion/exclusion list
  #inclusion_1: c("Typhimurium", "Hadar", "Muenchen"); 
  #inclusion_2_Cluster1_2016-2021: c("Typhimurium", "Hadar", "Muenchen","I 4,[5],12:i:-","Berta","Newport", "Saintpaul", "Enteritidis", "Litchfield", "Manhattan"); 
  #inclusion_3_Prop_Cluster1_2016-2021: c("Worthington1", "Uganda1", "Typhimurium1","Thompson1","Tennessee1","Senftenberg1","Schwarzengrund1", "Saintpaul1", "Ruiru1", "Rissen1", "Reading1", "Panama1", "Orion1", "Ohio1", "Newport1", "Muenster1", "Muenchen1", "Montevideo1", "Minnesota1", "Meleagridis1", "Mbandaka1", "Manhattan1", "London1", "Liverpool1", "Litchfield1", "Kentucky1", "Johannesburg1", "Javiana1", "Infantis1", "Indiana1", "IIIa [1],13,23:g,z51:-1", "I 4,[5],12:i:-1", "Holcomb1", "Heidelberg1", "Hadar1", "Enteritidis1", "Cubana1", "Cerro1", "Bredeney1", "Braenderup1", "Berta1", "Anatum1", "Albany1", "Agona1", "Alachua1", "4,[5],12:d:-1", "3,10:e,h:-1")
  ...
  use_attenuation = TRUE, # Use attenuation?
  use_scaling_factor = TRUE, # Use scaling factor?; DO NOT CHANGE
  comment = "m3_variable_prop_2021_list2_sf2_check" # User/Scenario comment; HIGHLY RECOMMENDED TO EDIT AS APPROPRIATE
)
m3_variable_attenuation_scaled_specifics <- get_table_values(m3_variable_attenuation_scaled) # Extracts specific data needed for Kim et al. 2024 paper
mean(m3_variable_attenuation_scaled$tdr_mean_pr_ill)
```

```{r Variable Attenuation & Scaled Run for comminuted turkey, HACCP baseline (2016-2021) list 3}
# Parameter setup ----
# NOTE: You can put these number straight into poultry_model_clustered() instead of defining them here, but this method lets you cite data sources more cleanly
salmonella_mean <- -4.857 #FSIS HACCP Data 2016-2021
salmonella_st_dev <- 2.052 # Now is Bayesian fitting, for FSIS HACCP Data 2016-2021, use 2.333
...
```{r Variable Attenuation & Scaled Run for comminuted turkey, HACCP new (2023-2024) list 1}
# Parameter setup ----
# NOTE: You can put these number straight into poultry_model_clustered() instead of defining them here, but this method lets you cite data sources more cleanly
salmonella_mean <- -4.607818  #FSIS HACCP Data 2023-2024
salmonella_st_dev <- 2.206763 #FSIS HACCP Data 2023-2024
...
m3_variable_attenuation_scaled <- poultry_model_clustered(
  ...
  cores = 12, # cores, total number of CPU cores to use in parallel, if you have fewer than the number specified the model should default to maximum available instead; MOST PCS SHOULD HAVE 12, IF UNSURE USE 12 OR detectCores()
  model_mode = 3, # model_mode: 1 = use Beta-Poisson model and parameters from WHO/FAO (2002) for all serotypes, 2 = use Beta-Poisson model with parameters from WHO/FAO (2002)  for all serotypes that are not Typhimurium - Possible Vaccine Strain, 3 = use Serotype-specific DR parameters collected from Teunis (2022); MODEL MODE MUST BE 1, 2, OR 3, USE AS APPROPRIATE
  serotype_probabilities = serotype_probabilities_2024, # dataframe with serotype probabilities; CAN BE SUBSTITUTED WITH A DIFFERENT DATAFRAME, SEE TOP OF setup
  serotype_equivalents = serotype_equivalents, # dataframe with how to serotype equivalences for Tuenis submodel; CAN BE SUBSTITUTED WITH A DIFFERENT DATAFRAME, SEE TOP OF setup
  salmonella_mean = salmonella_mean, # Log10 Salmonella dose mean; CAN BE CHANGED WITH A DIFFERENT VALUE
  salmonella_st_dev = salmonella_st_dev, # Log10 Salmonella dose standard deviation; CAN BE CHANGED WITH A DIFFERENT VALUE
  attenuation_input = c(-5, 1.91), # FSIS Variable attenuation, Quantitative Risk Assessment for Salmonella in Raw Chicken and Raw Chicken Products, page 42 
  inclusion_list_input=c("Typhimurium", "Hadar", "I 4,[5],12:i:-"), #c("Typhimurium", "Hadar", "Muenchen"),# Change the inclusion list based on different scenarios; if change to list 3, remember to change the stringr::str_sub for the inclusion/exclusion list
  #inclusion_1: c("Typhimurium", "Hadar", "Muenchen"); 
  #inclusion_2_Cluster1_2023-2024: c("Typhimurium", "Hadar", "Muenchen","I 4,[5],12:i:-", "Enteritidis", "Newport"); 
  #inclusion_3_Prop_Cluster1_2023-2024: c("Hadar1", "Agona1", "Infantis1","Schwarzengrund1","Reading1","Senftenberg1","Uganda1", "I 4,[5],12:i:-1", "Typhimurium1", "Brandenburg1", "Typhimurium - Possible Vaccine Strain1", "Albany1", "Enteritidis1", "Muenchen1", "Montevideo1", "Worthington1", "Alachua1", "Heidelberg1", "Johannesburg1", "Newport1", "Orion1", "Ouakam1", "Anatum1", "Barranquilla1", "Cubana1", "Gateshead1", "Kentucky1", "Muenster1")
  exclusion_list_input=c("Kentucky"), #c("Typhimurium - Possible Vaccine Strain", change to "Vaccined" in the proportion excel)
  ...
  use_attenuation = TRUE, # Use attenuation?
  use_scaling_factor = TRUE, # Use scaling factor?; DO NOT CHANGE
  comment = "m3_variable_prop_2024_list1_Kentucky" # User/Scenario comment; HIGHLY RECOMMENDED TO EDIT AS APPROPRIATE
)
m3_variable_attenuation_scaled_specifics <- get_table_values(m3_variable_attenuation_scaled) # Extracts specific data needed for Kim et al. 2024 paper
mean(m3_variable_attenuation_scaled$tdr_mean_pr_ill)
```

```{r Variable Attenuation & Scaled Run for comminuted turkey, HACCP new (2023-2024) list 2}
# Parameter setup ----
# NOTE: You can put these number straight into poultry_model_clustered() instead of defining them here, but this method lets you cite data sources more cleanly
salmonella_mean <- -4.607818  #FSIS HACCP Data 2023-2024
salmonella_st_dev <- 2.206763 #FSIS HACCP Data 2023-2024
...
m3_variable_attenuation_scaled <- poultry_model_clustered(
  ...
  use_attenuation = TRUE, # Use attenuation?
  use_scaling_factor = TRUE, # Use scaling factor?; DO NOT CHANGE
  comment = "m3_variable_prop_2024_list2" # User/Scenario comment; HIGHLY RECOMMENDED TO EDIT AS APPROPRIATE
)
m3_variable_attenuation_scaled_specifics <- get_table_values(m3_variable_attenuation_scaled) # Extracts specific data needed for Kim et al. 2024 paper
mean(m3_variable_attenuation_scaled$tdr_mean_pr_ill)
```

```{r Variable Attenuation & Scaled Run for comminuted turkey, HACCP new (2023-2024) list 3}
# Parameter setup ----
# NOTE: You can put these number straight into poultry_model_clustered() instead of defining them here, but this method lets you cite data sources more cleanly
salmonella_mean <- -4.607818  #FSIS HACCP Data 2023-2024
salmonella_st_dev <- 2.206763 #FSIS HACCP Data 2023-2024
...
m3_variable_attenuation_scaled <- poultry_model_clustered(
  ...
  salmonella_st_dev = salmonella_st_dev, # Log10 Salmonella dose standard deviation; CAN BE CHANGED WITH A DIFFERENT VALUE
  attenuation_input = c(-5, 1.91), # FSIS Variable attenuation, Quantitative Risk Assessment for Salmonella in Raw Chicken and Raw Chicken Products, page 42 
  inclusion_list_input=c("Hadar1", "Agona1", "Infantis1","Schwarzengrund1","Reading1","Senftenberg1","Uganda1", "I 4,[5],12:i:-1", "Typhimurium1", "Brandenburg1", "Typhimurium - Possible Vaccine Strain1", "Albany1", "Enteritidis1", "Muenchen1", "Montevideo1", "Worthington1", "Alachua1", "Heidelberg1", "Johannesburg1", "Newport1", "Orion1", "Ouakam1", "Anatum1", "Barranquilla1", "Cubana1", "Gateshead1", "Kentucky1", "Muenster1"),# Change the inclusion list based on different scenarios; if change to list 3, remember to change the stringr::str_sub for the inclusion/exclusion list
  #inclusion_1: c("Typhimurium", "Hadar", "Muenchen"); 
  #inclusion_2_Cluster1_2023-2024: c("Typhimurium", "Hadar", "Muenchen","I 4,[5],12:i:-", "Enteritidis", "Newport"); 
  #inclusion_3_Prop_Cluster1_2023-2024: c("Hadar1", "Agona1", "Infantis1","Schwarzengrund1","Reading1","Senftenberg1","Uganda1", "I 4,[5],12:i:-1", "Typhimurium1", "Brandenburg1", "Typhimurium - Possible Vaccine Strain1", "Albany1", "Enteritidis1", "Muenchen1", "Montevideo1", "Worthington1", "Alachua1", "Heidelberg1", "Johannesburg1", "Newport1", "Orion1", "Ouakam1", "Anatum1", "Barranquilla1", "Cubana1", "Gateshead1", "Kentucky1", "Muenster1")
  exclusion_list_input=c("Kentucky"), #c("Typhimurium - Possible Vaccine Strain", change to "Vaccined" in the proportion excel)
  ...
  use_attenuation = TRUE, # Use attenuation?
  use_scaling_factor = TRUE, # Use scaling factor?; DO NOT CHANGE
  comment = "m3_variable_prop_2024_list3" # User/Scenario comment; HIGHLY RECOMMENDED TO EDIT AS APPROPRIATE
)
m3_variable_attenuation_scaled_specifics <- get_table_values(m3_variable_attenuation_scaled) # Extracts specific data needed for Kim et al. 2024 paper
mean(m3_variable_attenuation_scaled$tdr_mean_pr_ill)
```

```{r Variable Attenuation & Scaled Run for comminuted turkey, HACCP new (2023-2024) list 2&Exclusion list}
# Parameter setup ----
# NOTE: You can put these number straight into poultry_model_clustered() instead of defining them here, but this method lets you cite data sources more cleanly
salmonella_mean <- -4.607818  #FSIS HACCP Data 2023-2024
salmonella_st_dev <- 2.206763 #FSIS HACCP Data 2023-2024
...
m3_variable_attenuation_scaled <- poultry_model_clustered(
  ...
  salmonella_st_dev = salmonella_st_dev, # Log10 Salmonella dose standard deviation; CAN BE CHANGED WITH A DIFFERENT VALUE
  attenuation_input = c(-5, 1.91), # FSIS Variable attenuation, Quantitative Risk Assessment for Salmonella in Raw Chicken and Raw Chicken Products, page 42 
  inclusion_list_input=c("Typhimurium", "Hadar", "Muenchen","I 4,[5],12:i:-", "Enteritidis", "Newport"),# Change the inclusion list based on different scenarios; if change to list 3, remember to change the stringr::str_sub for the inclusion/exclusion list
  #inclusion_1: c("Typhimurium", "Hadar", "Muenchen"); 
  #inclusion_2_Cluster1_2023-2024: c("Typhimurium", "Hadar", "Muenchen","I 4,[5],12:i:-", "Enteritidis", "Newport"); 
  #inclusion_3_Prop_Cluster1_2023-2024: c("Hadar1", "Agona1", "Infantis1","Schwarzengrund1","Reading1","Senftenberg1","Uganda1", "I 4,[5],12:i:-1", "Typhimurium1", "Brandenburg1", "Typhimurium - Possible Vaccine Strain1", "Albany1", "Enteritidis1", "Muenchen1", "Montevideo1", "Worthington1", "Alachua1", "Heidelberg1", "Johannesburg1", "Newport1", "Orion1", "Ouakam1", "Anatum1", "Barranquilla1", "Cubana1", "Gateshead1", "Kentucky1", "Muenster1")
  exclusion_list_input=c("Adelaide","Agbeni","Agona","Alachua","Albany","Anatum","Bareilly","Braenderup","Brandenburg","Calabar","Carrau","Cerro","Chester","Concord","Cubana","Duisburg","Eastbourne","Gaminara","Gateshead","Give","Goldcoast","Hartford","Heidelberg","Hvittingfoss","Idikan","Infantis","Javiana","Johannesburg","Vaccined","Kiambu","Koessen","Leiden","Livingstone","Lomalinda","Lubbock","Mbandaka","Meleagridis","Miami","Mississippi","Montevideo","Muenster","Norwich","Ohio","Okatie","Oranienburg","Panama","Poona","Potsdam","Reading","Rissen","Rubislaw","Sandiego","Schwarzengrund","Senftenberg","Telelkebir","Thompson","Uganda","Urbana","Vinohrady","Virchow","Weltevreden"), #c("Typhimurium - Possible Vaccine Strain", change to "Vaccined" in the proportion excel)
  scaling_factor = 0.00025,
  product_consumed_per_individual_per_day_mean = product_consumed_per_individual_per_day_mean, # Log10 Product consumed mean; CAN BE CHANGED IF YOU HAVE NEW DATA, OTHERWISE USE CDC 2016, Lambertini  et al. (2019)
  product_consumed_per_individual_per_day_st_dev = product_consumed_per_individual_per_day_st_dev, # Log10 Product consumed standard deviation; CAN BE CHANGED IF YOU HAVE NEW DATA, OTHERWISE USE CDC 2016, Lambertini  et al. (2019)
  annual_servings = 10983387032, #28482586137, # total turkey servings per year in the U.S; POPULATION*NUMBER OF SERVINGS PER PERSON, CURRENTLY USES 2023 DATA
  ...
  use_attenuation = TRUE, # Use attenuation?
  use_scaling_factor = TRUE, # Use scaling factor?; DO NOT CHANGE
  comment = "m3_variable_prop_2024_list2_Exclusion" # User/Scenario comment; HIGHLY RECOMMENDED TO EDIT AS APPROPRIATE
)
m3_variable_attenuation_scaled_specifics <- get_table_values(m3_variable_attenuation_scaled) # Extracts specific data needed for Kim et al. 2024 paper
mean(m3_variable_attenuation_scaled$tdr_mean_pr_ill)
m3_variable_attenuation_scaled_specifics
```



```{r}
#check baseline P(ill)
mean(m3_variable_attenuation_scaled$tdr_mean_pr_ill)
View(m3_variable_attenuation_scaled)
```
